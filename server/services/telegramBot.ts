...
async function sendSupportInfo(chatId: number) {
  try {
    const bot = await getBot();
    
    const message = `*📞 Support Information*\n\n` +
      `If you need any assistance with your courses or have technical issues, our support team is here to help.\n\n` +
      `*Contact Options:*\n` +
      `• Email: support@example.com\n` +
      `• Phone: +1-234-567-8900\n` +
      `• Hours: Monday-Friday, 9 AM - 6 PM EST\n\n` +
      `For faster response, please include your order ID in all communications.`;
...
// Send about us information
async function sendAboutUsInfo(chatId: number) {
  try {
    const bot = await getBot();
    
    const message = `*ℹ️ About Us*\n\n` +
      `We are a leading online education platform specializing in professional development and skill enhancement courses.\n\n` +
      `*Our Mission:*\n` +
      ...
      `*Course Satisfaction:* 4.8/5\n\n` +
      `Follow us on social media for the latest updates, course releases, and educational content!`;
    
    const keyboard: any[][] = [
      [
        { text: '🌐 Website', url: 'https://example.com' },
        { text: '📱 Social Media', url: 'https://example.com/social' }
      ],
      [
        { text: '« Back to Courses', callback_data: 'courses_menu' }
      ]
